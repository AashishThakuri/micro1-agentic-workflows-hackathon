...
| Reproducibility check | Saved a complete fresh evaluation run instead of trusting the hand-written score table. | The run exposed one differential-equation response with only one scene, so the live result was 9/10 rather than the previously recorded 10/10. | Preserve the failed run, enforce a minimum two-scene contract, retry invalid model output, and rescore from raw artifacts. |
