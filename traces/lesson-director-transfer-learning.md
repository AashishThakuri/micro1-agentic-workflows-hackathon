...
| 2 | Gemini structured generation receives the source, visual language, renderer grammar, and JSON schema. | HTTP 200 with `Understanding Transfer Learning` and three scenes: `The Base Model`, `Transferring Knowledge`, and `Freezing vs Fine-tuning`. | Validate and normalize every scene. |
...
| 5 | Narration tool receives one scene title and exact narration at a time. | Kore voice returns WAV audio; requests are serialized to respect quota. | Cache scene audio and synchronize playback progress to subtitles and animation. |
...
A three-scene transfer-learning lesson with typed evidence for every stage: structured plan, renderer selection, visual objects, animation beats, direct manipulation, narration, and playback controls.
